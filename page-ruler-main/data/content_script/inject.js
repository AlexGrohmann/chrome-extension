        ...
        move: function (e) {
          ...
          if (action) {
            if (config.ruler.element.rect) {
              ...
              /*  */
              config.ruler.element.rect.setAttribute(
                "colorful",
                config.options.colorful
              );
              /*  */
              if (config.options.colorful) {
                ...
                inrange.ex =
                  config.ruler.current.x < window.innerWidth - text.width * 1;
                inrange.ey =
                  config.ruler.current.y < window.innerHeight - text.height * 1;
                /*  */
                center.condition.w =
                  width > text.width * 2 + 2 * text.adjustment;
                center.condition.h =
                  height > text.height * 3 + 4 * text.adjustment;
                center.container.x =
                  (config.ruler.start.x + config.ruler.current.x) / 2;
                center.container.y =
                  (config.ruler.start.y + config.ruler.current.y) / 2;
                center.placement.x =
                  center.condition.w && center.condition.h
                    ? center.container.x - text.width - text.spacing
                    : (center.container.x +
                        (window.innerWidth / 2 - text.width - text.spacing)) /
                      2;
                center.placement.y =
                  center.condition.w && center.condition.h
                    ? center.container.y - (text.height * 3) / 2 - text.spacing
                    : (center.container.y +
                ...
                /*  */
                const cond_0 =
                  inrange.sx && inrange.sy && inrange.ex && inrange.ey;
                ...
                /*  */
                text.a.style.left = visible
                  ? 0
                  : center.placement.x + text.width + text.spacing + "px";
                text.a.style.top = visible
                  ? -1 * (text.height + text.spacing) + "px"
                  : center.placement.y + "px";
                /*  */
                text.b.style.left = visible
                  ? -1 * (text.width + text.spacing) + "px"
                  : center.placement.x + "px";
                text.b.style.top = visible ? 0 : center.placement.y + "px";
                /*  */
                text.c.style.left = visible
                  ? width / 2 - text.width / 2 - 2 * text.adjustment + "px"
                  : center.placement.x + "px";
                text.c.style.top = visible
                  ? 0
                  : center.placement.y +
                    1 * (text.height + text.spacing) +
                    "px";
                /*  */
                text.d.style.left = visible
                  ? 0
                  : center.placement.x + text.width + text.spacing + "px";
                text.d.style.top = visible
                  ? height / 2 - text.height / 2 - 2 * text.adjustment + "px"
                  : center.placement.y +
                    1 * (text.height + text.spacing) +
                    "px";
                /*  */
                text.e.style.left = visible
                  ? width - text.adjustment + "px"
                  : center.placement.x + "px";
                text.e.style.top = visible
                  ? height -
                    (text.height + text.spacing + text.adjustment) +
                    "px"
                  : center.placement.y +
                    2 * (text.height + text.spacing) +
                    "px";
                /*  */
                text.f.style.left = visible
                  ? width - (text.width + text.spacing + text.adjustment) + "px"
                  : center.placement.x + text.width + text.spacing + "px";
                text.f.style.top = visible
                  ? height - text.adjustment + "px"
                  : center.placement.y +
                    2 * (text.height + text.spacing) +
                    "px";
                /*  */
                ...
                if (visible) {
                  ...
                  config.ruler.element.section.style.width =
                    text.width * 2 + text.spacing + text.adjustment + 1 + "px";
                  config.ruler.element.section.style.height =
                    text.height * 3 +
                    2 * text.spacing +
                    text.adjustment +
                    1 +
                    "px";
                  /*  */
                  config.ruler.element.rect.appendChild(
                    config.ruler.element.section
                  );
                }
              } else {
                const metrics = {};
